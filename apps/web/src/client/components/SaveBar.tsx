...
export function SaveBar({ dirty, saving = false }: { dirty?: boolean; saving?: boolean }) {
  ...
  return (
    <div className="sticky bottom-4 z-30 flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-slate-950/85 px-4 py-3 shadow-[0_8px_30px_rgba(0,0,0,0.4)] backdrop-blur-xl">
      <span className={`text-sm ${gated && dirty ? 'font-semibold text-blue-300' : 'text-slate-500'}`}>
        {gated ? (dirty ? t('settings.unsaved') : t('settings.allSaved')) : ''}
      </span>
      <button
        type="submit"
        disabled={saving || (gated && !dirty)}
        className="rounded-xl bg-gradient-to-r from-blue-500 via-blue-500 to-blue-400 px-6 py-2.5 font-semibold text-slate-950 shadow-[0_0_20px_-6px_rgba(59,130,246,0.7)] transition hover:scale-[1.02] hover:shadow-[0_0_26px_-4px_rgba(59,130,246,0.8)] disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:scale-100"
      >
        {t('settings.save')}
      </button>
    </div>
