...
const SYSTEM_PROMPT = [
  ...
  '- ICONS: give EVERY category and EVERY channel EXACTLY ONE leading emoji — never zero, never two or more, and no emoji',
  '  anywhere except the very front. Pick an emoji that genuinely fits the name/purpose. If a name ALREADY has an emoji,',
  '  judge whether it fits: keep it when it does, otherwise replace it with a better one — the result must still have exactly',
  '  ONE leading emoji. Use a consistent, tasteful style; never use the same emoji for everything.',
  '  Category emoji read like section headers (e.g. 📌 💬 🎮 🎨 🎧 🎫 🔊).',
  '- NAMES: preserve each name\'s original language — NEVER translate. You MAY fix spelling mistakes, tidy casing and spacing,',
  '  drop redundant words, and apply a consistent naming style — but keep each name recognizable and its meaning intact.',
